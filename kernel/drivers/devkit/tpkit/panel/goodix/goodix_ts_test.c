...
static inline long gt9xx_calc_resisitance(u16 self_data,u8 master,u8 slave,u16 short_code,u8 flag)
{
	...
	else	//Others
	{
	    ...
	    if (flag == 1)   //GT950分压式，DACA必须处理成ABIST
	    {
				...
				u16 short_code, u8 pos)
{
	...
	if ((short_code & (0x8000)) == 0)	//short to GND
		...
		r = div_s64(((52662850)*10), (short_code & (~0x8000))) - 40 * FLOAT_AMPLIFIER * 10;
	else			//short to VDD
		//r = (long long)40 * 9 * 1024 * (avdd_value - 9) * FLOAT_AMPLIFIER / ((short_code & (~0x8000)) * 7) - 40 * FLOAT_AMPLIFIER * 10;
		r = div_s64((40 * 9 * 1024 * (avdd_value - 9) * FLOAT_AMPLIFIER),((short_code & (~0x8000)) * 7)) - 40 * FLOAT_AMPLIFIER * 10;

	//r = r / FLOAT_AMPLIFIER;
	r = div_s64(r, FLOAT_AMPLIFIER);
	if(r > 65535)
	{
		r = 65535;
	}
