...
static void chg_dsm_external_power_changed(struct power_supply *psy)
{
	...
	int soc = INVALID_VALUE;
	int usb_pres = INVALID_VALUE;
	int batt_volt = INVALID_VALUE;
	int batt_curr = INVALID_VALUE;
	int batt_temp = INVALID_VALUE;
	...
	int soc_now = dsm_get_property_from_psy(chip->batt_psy, POWER_SUPPLY_PROP_CAPACITY);
	int usb_pres_now = dsm_get_property_from_psy(chip->usb_psy, POWER_SUPPLY_PROP_PRESENT);
	int batt_volt_now = dsm_get_property_from_psy(chip->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW);
	int batt_curr_now = dsm_get_property_from_psy(chip->batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW);
	int batt_temp_now = dsm_get_property_from_psy(chip->batt_psy, POWER_SUPPLY_PROP_TEMP);
	...
	if (abs(soc - soc_now) >= SOC_JUMP_STEP) {
