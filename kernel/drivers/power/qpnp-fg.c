				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		chip->batt_max_voltage_uv = val->intval;
		if (chip->batt_max_voltage_uv < 4200000) {
			fg_sram_update_period_ms = 3000;
			set_prop_jeita_temp(chip, FG_MEM_HARD_HOT, 950);
			set_prop_jeita_temp(chip, FG_MEM_SOFT_HOT, 900);
			set_prop_jeita_temp(chip, FG_MEM_SOFT_COLD, -200);
			set_prop_jeita_temp(chip, FG_MEM_HARD_COLD, -250);
        }
		break;
#endif
...
wait:
	...
	pr_info("Qpnp-fg fg_batt_type:%s\n",chip->batt_type);
...
static int fg_restart_set(const char *val, const struct kernel_param *kp)
{
	...
	if (global_fg_chip == NULL){
		pr_info("qpnp fg chip not init, return directly\n");
		return 0;
	}
