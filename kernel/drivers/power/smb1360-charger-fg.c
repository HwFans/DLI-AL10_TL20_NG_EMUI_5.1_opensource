...
/*lint -save -esym(528, chg_no_timer_set) -esym(528, __param_chg_no_timer) -esym(528, kernel_param_ops)*/
...
static int charging_current_now_set=1500;
...
static int smb1360_safety_timer_enable(struct smb1360_chip *chip, bool enable)
{
	int rc = 0;
	u8 reg = 0;
	if (chg_no_timer)
	...
	{
		reg = SAFETY_TIME_DISABLE_BIT;
	}
	rc = smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
			SAFETY_TIME_DISABLE_MASK, reg);
	if (rc < 0)
	{
		dev_err(chip->dev,
...
static int chg_no_timer_set(const char *val, const struct kernel_param *kp)
{
	...
	chip = container_of(batt_psy, struct smb1360_chip, batt_psy);
	...
	return smb1360_safety_timer_enable(chip, !chg_no_timer);
}
