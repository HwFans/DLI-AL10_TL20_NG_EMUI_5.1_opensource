...
static int smbchg_default_dcp_icl_ma = 1500; // 1800 -> 1500
module_param_named(
...
#ifdef CONFIG_HW_TEMP_TEST
static int hw_temp_test_mask = 0;
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp CAP limit version,capacity=%d\n",capacity);
		if(capacity <= 3) {
			capacity = 3;
		}
	}
...
static int get_prop_batt_temp(struct smbchg_chip *chip)
{
	...
	/* Delhi need 60 degree reboot when EMUI reboot at 68 degree */
	if (temp >= 600)
		temp += 80;
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		temp = 290;
		pr_smb(PR_STATUS, "Temp TEMP limit version,temp=%d\n",temp);
	}
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp get voltage uv = %d\n", uv);
		if (uv < 3500000) {
			uv = 3500000;
		}
	}
...
#define OTG_ILIMIT_BIT_MASK (BIT(1) | BIT(0))
#define OTG_ILIMIT_250MA 0x0
#define OTG_ILIMIT_600MA 0x1
#define OTG_ILIMIT_750MA 0x2
#define OTG_ILIMIT_1000MA 0x3
static int smbchg_regulator_init(struct smbchg_chip *chip)
{
	...
	// Modify default OTG current.
	rc = smbchg_sec_masked_write(chip,
				chip->otg_base + OTG_ICFG,
				OTG_ILIMIT_BIT_MASK, OTG_ILIMIT_1000MA);
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp batt_hot_handler\n");
		return IRQ_HANDLED;
	}
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp batt_cold_handler\n");
		return IRQ_HANDLED;
	}
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp batt_warm_handler\n");
		return IRQ_HANDLED;
	}
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
	if (hw_temp_test_mask >= 1) {
		pr_smb(PR_STATUS, "Temp batt_cool_handler\n");
		return IRQ_HANDLED;
	}
#endif
...
#ifdef CONFIG_HW_TEMP_TEST
//static int hw_temp_test_mask = 0;
static int set_hw_temp_test_mask(const char *val, struct kernel_param *kp)
{
	int ret;
	struct smbchg_chip *chip = global_chip;
	...
	if (chip) {
		pr_info("set_hw_temp_test_mask  to %d\n", hw_temp_test_mask);
		if (hw_temp_test_mask >= 2) {
			set_property_on_fg(chip, POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN, 4100000);
			smbchg_float_voltage_set(chip, 4100);
		} else {
		//	set_property_on_fg(chip, POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN, 4400000);
		//	smbchg_float_voltage_set(chip, 4400);
		}
		if (hw_temp_test_mask >= 1) {
		...
		set_property_on_fg(chip, POWER_SUPPLY_PROP_UPDATE_NOW, 1);
		cancel_delayed_work_sync(&chip->update_period_work);
...
module_param_call(hw_temp_test_mask, set_hw_temp_test_mask, param_get_uint,
					&hw_temp_test_mask, 0664);
#endif
...
static char DUMP_OTG[] = {0x08, 0xf3};
static char DUMP_BAT_IF[] = {0x08, 0x10, 0x15, 0x18, 0x42, 0xf3, 0xfd};
static char DUMP_USB_CHGPATH[] = {0x07, 0x08, 0x09, 0x10, 0x15, 0x18, 0x0d, 0x40, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xfd};
				      ...
				      char *offset, int length)
{
	...
	strncat_length_protect(reg_value, "| ");
}
				      ...
				      char *offset, int length)
{
	...
	for (i = 0; i < length; i++) {
		snprintf(buff, BUFFER_SIZE, "R[0x%04x] ", base_addr + offset[i]);
		strncat_length_protect(reg_head, buff);
	}
	strncat_length_protect(reg_head, "| ");
}
